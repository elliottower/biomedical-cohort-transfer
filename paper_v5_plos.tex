...
\title{Grassmannian geodesic distance predicts cross-cohort classifier degradation under analytical heterogeneity, after controlling for source classifier quality}
...
Because the effective sample size for partial correlation is the number of independent cohorts ($C$), not the number of pairs ($C(C-1)$), statistical power is determined by how many cohorts each dataset contains. Using the Fisher $z$-transform approximation with $df = C - 3$ (one covariate), we computed the minimum detectable partial $\rho$ at 80\% power ($\alpha = 0.05$, two-sided) for each dataset (Table~\ref{tab:power}). Monte Carlo simulation (10{,}000 replicates) verified the asymptotic results.

The CRC dataset ($C = 9$) has 41\% power to detect its own observed effect ($\rho = 0.61$); detecting a partial $\rho$ of $0.61$ at 80\% power requires approximately 19 independent studies. The QMDiab dataset ($C = 9$) has 18\% power at its observed $\rho = 0.40$, requiring approximately 47 studies for 80\% power. The IBD dataset ($C = 5$) cannot detect any partial correlation below $\rho = 0.95$, making its null result uninformative about effect sizes in the range observed on CRC. These power limitations are intrinsic to the partial-correlation design and cannot be resolved without additional independent cohort collections.

\begin{table}[t]
\centering
\caption{Power analysis. Effective $n$ is the number of independent cohorts. MDE is the minimum detectable effect (partial $\rho$) at 80\% power. Power at observed $\rho$ is Monte Carlo-estimated (10{,}000 replicates).}
\label{tab:power}
\begin{tabular}{lcccc}
\toprule
Dataset & $C$ (eff.\ $n$) & MDE ($\rho$, 80\%) & Obs.\ partial $\rho$ & Power at obs.\ $\rho$ \\
\midrule
CRC microbiome & 9 & 0.85 & $+$0.61 & 41\% \\
QMDiab & 9 & 0.85 & $+$0.40 & 18\% \\
IBD & 5 & --- & $-$0.11 & 5\% \\
SPIROMICS COPD & 8 & 0.89 & $+$0.08 & 6\% \\
Breast cancer GEO & 7 & 0.93 & $-$0.004 & --- \\
TCGA-BRCA & 14 & 0.71 & $-$0.12 & 7\% \\
MTBLS7260 & 15 & 0.69 & $+$0.04 & 5\% \\
\bottomrule
\end{tabular}
\end{table}
...
Decomposing the 72 ordered pairs by type clarifies the source of this signal. Cross-biofluid pairs (54 pairs, comparing different biological matrices) show both higher mean geodesic distance (3.52 vs 2.49) and larger mean AUC gap ($+0.211$ vs $+0.022$) than within-biofluid cross-ethnicity pairs (18 pairs). Within each subset, the partial correlation is null: cross-biofluid partial $\rho = -0.04$, within-biofluid cross-ethnicity partial $\rho = +0.14$. The overall partial $\rho = +0.40$ thus reflects the between-type contrast---pairs crossing biological matrices have both higher subspace divergence and larger classifier degradation---rather than a graded, within-type prediction. This is consistent with the boundary condition thesis: the geodesic captures analytical heterogeneity (different biofluids = different data-generating processes), and within a single biofluid the cross-ethnicity variation does not produce sufficient subspace divergence for the geodesic to predict degradation.
...
The geodesic distances are also compressed (CV 3.5\%), reflecting the mild batch regime of plates within a single study. With 15 plates (effective $n = 15$), the minimum detectable partial $\rho$ at 80\% power is 0.69 (Table~\ref{tab:power}). Taken together, the metabolomics null is consistent with insufficient statistical power for both the source quality signal and the geometric signal, rather than absence of the underlying relationship.
...
\paragraph{IBD metagenomics (5 studies, 20 pairs).} Raw Spearman correlation is $\rho = +0.09$ ($p = 0.70$); the partial correlation controlling for source AUC is $\rho = -0.11$, with clustered bootstrap CI $[-0.90, +0.66]$ (39\% positive). Geodesic distances are compressed into a narrow range (3.05--3.75), and the LOO analysis shows that adding geodesic distance to the source-quality model \emph{increases} MAE by 18\%. Two factors explain this null. First, with only 5 studies the effective sample size is too small to detect any partial correlation below $\rho = 0.95$ at 80\% power (Table~\ref{tab:power}); the IBD null is consistent with effect sizes as large as the CRC result. Second, IBD microbiome composition varies substantially with disease activity state---patients in remission versus flare may differ more from each other than across studies---so within-study variance likely masks any between-study geometric signal. Two studies (HMP2, Lewis 2015) also have severe class imbalance ($\leq$26 controls), producing noisy AUC estimates. The null is therefore uninformative about the true effect size: both ``no effect'' and ``moderate effect masked by low power and high within-cohort variance'' are consistent with the data.
...
The two positive datasets differ in which geometric measure performs best. On the CRC data, geodesic distance is the strongest predictor ($\Delta R^2 = +0.23$), outperforming centroid distance ($\Delta R^2 = +0.08$). On QMDiab, centroid distance dominates (partial $\rho = +0.66$) because the cross-biofluid comparison---plasma versus urine versus saliva---produces large centroid shifts in metabolite concentrations that a simple Euclidean distance captures. The QMDiab pair-type decomposition (\S\ref{sec:qmdiab}) reveals that the overall partial $\rho = +0.40$ reflects the contrast between cross-biofluid pairs (high geodesic, high gap) and within-biofluid cross-ethnicity pairs (low geodesic, low gap), rather than a graded prediction within either category. The CRC data, where all 72 pairs involve the same data type (metagenomic species abundances) and the geodesic varies continuously across study pairs, provides the cleaner test of subspace-level geometry as a graded predictor.

The five null results identify four distinct failure modes, though several are confounded with low statistical power (Table~\ref{tab:power}). First, centralized processing (SPIROMICS, TCGA) eliminates between-cohort analytical variation entirely. Second, shared measurement platforms with insufficient analytical divergence (breast cancer GEO on Affymetrix HG-U133A) produce subspace geometry that is too similar across cohorts to predict transportability, even when the studies are otherwise independent. Third, high within-cohort disease-activity variance combined with severe underpowering (IBD: 5 studies, power $< 5\%$ at any detectable effect) masks any between-study geometric signal; this null is uninformative about effect sizes in the CRC range. Fourth, insufficient statistical power for both AUC estimation and partial correlation (MTBLS7260: AUC SE $\sim 0.10$ from $\sim$12 positives per plate) prevents resolution of either source quality or geometry.
...
\paragraph{Limitations.} The effective sample size for our correlation analyses is the number of studies ($C = 9$ for CRC), not the number of pairs ($C(C-1) = 72$), because pairs sharing a source or target are not independent. The power analysis (Table~\ref{tab:power}) shows that detecting $\rho = 0.61$ at 80\% power requires approximately 19 independent studies; our CRC dataset has 9, yielding only 41\% power. Our clustered bootstrap accounts for the dependence structure, producing appropriately wide CIs (e.g., $[+0.03, +0.77]$ for CRC with random forest, compared to $[+0.44, +0.75]$ from naive pair-level resampling). Several additional caveats apply. First, all seven datasets are retrospective analyses of public data; a prospective, preregistered replication on a held-out multi-study collection would strengthen the positive claim. Second, the QMDiab result is driven by the between-type contrast (cross-biofluid vs within-biofluid pairs) rather than graded within-type prediction, limiting its interpretation as a continuous predictor. Third, the IBD analysis has only 5 studies, making its null uninformative about effect sizes below $\rho = 0.95$. Fourth, PCA subspaces capture linear variance structure; nonlinear manifold distances such as diffusion maps \citep{conway1996functions} might prove complementary. Fifth, the breast cancer gene expression studies use ESR1-based labeling for 3 of 7 studies; while the GMM separation is high ($\geq 2.17$), concordance with clinical ER status is imperfect ($\sim$90\%), which could attenuate classification performance.
...
\paragraph{Practitioner checklist.} The seven-dataset evaluation identifies three conditions that must hold for the geodesic method to provide useful transportability predictions:
\begin{enumerate}
\item \textbf{Analytical heterogeneity.} Cohorts must differ in measurement platform, protocol, or biological matrix. Centralized processing (SPIROMICS, TCGA) or shared microarray platforms (breast cancer GEO) eliminate the variation the geodesic measures. Independent study sites alone are insufficient without protocol-level divergence.
\item \textbf{Subspace separability.} Cohort PCA subspaces must be distinguishable from a label-permutation null ($z > 3$). All seven datasets satisfy this condition, confirming that PCA subspaces are geometrically meaningful even when prediction fails.
\item \textbf{Between-cohort dominance.} Between-cohort distributional shift must exceed within-cohort disease-state variance. IBD violates this condition: disease-activity variation within studies may exceed the geometric signal between studies.
\end{enumerate}
When all three conditions hold (CRC microbiome, QMDiab cross-biofluid), the geodesic augments source classifier quality to improve degradation prediction. When any condition fails, the method correctly returns a null result or operates as a batch detector rather than a transportability predictor.

\section*{Data availability statement}

All datasets are publicly available. CRC microbiome: Zenodo record 3517209 \citep{wirbel2019meta}. QMDiab: Figshare article 5904022 \citep{yousri2015metabolic}. MTBLS7260: MetaboLights accession MTBLS7260 \citep{leonletelier2024contributions}. IBD metagenomics: SIAMCAT Zenodo archive \citep{wirbel2021siamcat}. SPIROMICS COPD: Metabolomics Workbench ST002088 \citep{bowler2017spiromics}. Breast cancer gene expression: GEO platform GPL96 (accessions GSE2034, GSE7390, GSE1456, GSE11121, GSE4922, GSE6532, GSE25066). TCGA-BRCA: UCSC Xena and cBioPortal. Code and results are archived at Zenodo (DOI: \href{https://doi.org/10.5281/zenodo.21248764}{10.5281/zenodo.21248764}) and available at \url{https://github.com/elliottower/biomedical-cohort-transfer}.
...
Elliot Tower: Conceptualization, Methodology, Software, Formal analysis, Investigation, Data curation, Writing -- original draft, Writing -- review \& editing, Visualization.

\section*{Use of AI-assisted technologies}

Claude Code (Anthropic) was used as a coding assistant for data analysis, statistical computation, and manuscript preparation. Perplexity Pro was used for literature search and as an automated review assistant for pre-registration documents. Neither tool made scientific decisions. All experimental design, interpretations, and conclusions are the author's.
