...
\begin{abstract}
Classifiers trained on one cohort routinely degrade when applied to another, yet quantifying this risk before training requires a measure of cross-cohort distributional distance. We propose geodesic distance between cohort-specific PCA subspaces on the Grassmannian manifold $\mathrm{Gr}(k, d)$ as such a measure and test it on five datasets spanning metagenomics, metabolomics, and multi-site clinical studies. Raw geodesic--gap correlations are null or weak ($\rho \leq 0.34$) because the AUC gap conflates source classifier quality with distribution shift. After partialing out source internal AUC, geodesic distance predicts the residual gap on a colorectal cancer microbiome meta-analysis (9 studies, 824 samples; partial $\rho = +0.61$, clustered bootstrap 95\% CI $[-0.02, +0.80]$, 97\% positive; $\Delta R^2 = +0.23$) and on the QMDiab multi-biofluid metabolomics study (3 biofluids $\times$ 3 ethnicities, 356 participants; partial $\rho = +0.40$, clustered CI $[+0.04, +0.73]$). Leave-one-study-out prospective validation on the CRC data confirms out-of-sample utility: the geodesic-augmented model reduces prediction MAE by 42\% over baseline and 22\% over a source-quality-only model (mean LOO $\rho = 0.80$). Two additional datasets---IBD metagenomics (5 studies) and COPD multi-site metabolomics (8 clinical sites)---yield null results, revealing interpretable boundary conditions: the method requires genuine distributional shift that is captured by the top-$k$ PCA subspace and exceeds within-cohort disease-state variance.
\end{abstract}
...
We test this idea on five datasets spanning different domains: a colorectal cancer (CRC) microbiome meta-analysis (9 independent studies across 7 countries; \citealp{wirbel2019meta}), a multi-biofluid metabolomics study (QMDiab, 3 biofluids $\times$ 3 ethnicities; \citealp{yousri2015metabolic}), a single-study metabolomics dataset (MTBLS7260, 15 analytical plates; \citealp{leonletelier2024contributions}), an inflammatory bowel disease (IBD) metagenomics meta-analysis (5 studies, 1{,}176 samples), and a COPD multi-site metabolomics study (SPIROMICS, 8 clinical sites, 584 samples; \citealp{bowler2017spiromics}).
...
\begin{itemize}
\item \textbf{Confound identification (\S\ref{sec:crc}).} The AUC gap conflates source classifier quality ($\rho = +0.68$ with internal AUC) and distribution shift. Raw geodesic--gap correlations are null or weak on all five datasets.
...
\item \textbf{Boundary conditions (\S\ref{sec:boundary}).} IBD metagenomics (5 studies, partial $\rho = -0.11$) and COPD multi-site metabolomics (8 sites, partial $\rho = +0.08$) yield null results. The method requires genuine distributional shift exceeding within-cohort disease-state variance; centralized analytical platforms eliminate the between-site variation the geodesic measures.
...
\begin{table}[t]
\centering
\caption{Dataset summary. Cohorts are independent studies (CRC, IBD), biofluid--ethnicity groups (QMDiab), analytical plates (MTBLS7260), or clinical sites (SPIROMICS).}
\label{tab:cohorts}
...
\subsection{Boundary conditions: IBD metagenomics and COPD metabolomics}
\label{sec:boundary}

Two additional datasets produce null results, identifying conditions under which the geodesic method fails.
...
These null results sharpen the method's scope. Geodesic distance on $\mathrm{Gr}(k,d)$ predicts transportability when three conditions hold: (1) genuine distributional shift exists between cohorts (different laboratories, platforms, or biological matrices); (2) the shift is captured by the top-$k$ PCA subspace rather than the mean alone; and (3) between-cohort variance exceeds within-cohort disease-state variance. Centralized analytical processing (SPIROMICS) eliminates condition (1); high within-cohort activity-state variance (IBD) violates condition (3).
...
The null results on IBD and COPD identify the method's boundary conditions. SPIROMICS uses a centralized Metabolon platform across all 8 clinical sites, producing negligible between-site analytical variation and compressed geodesic distances (0.54 span). The geodesic correctly measures no shift because there is none. IBD metagenomics has the opposite problem: distributional shift exists but is masked by within-study disease-activity variance (remission versus flare), which the top-$k$ PCA subspace captures preferentially over between-study effects. These two failure modes---absent shift and masked shift---clarify that the geodesic measures analytical and population-level distributional differences, and requires these to dominate the variance structure.

The MTBLS7260 null result illustrates a third failure mode: insufficient statistical power. With $\sim$12 positives per plate, AUC estimates have standard errors of $\sim$0.10, comparable to the total signal range, so neither source quality nor geometry can be resolved.

\paragraph{Limitations.} The effective sample size for our correlation analyses is the number of studies ($C = 9$ for CRC), not the number of pairs ($C(C-1) = 72$), because pairs sharing a source or target are not independent. Our clustered bootstrap accounts for this structure, producing appropriately wide CIs (e.g., $[+0.03, +0.77]$ for CRC with random forest, compared to $[+0.44, +0.75]$ from naive pair-level resampling). Several additional caveats apply. First, all five datasets are retrospective analyses of public data. Second, the QMDiab cohort structure (3 biofluids $\times$ 3 ethnicities) confounds biofluid effects with ethnicity effects for the smallest group (Indian, $n = 34$). Third, the IBD analysis has only 5 studies (20 pairs), limiting statistical power; larger IBD meta-analyses might reveal a weak geometric signal. Fourth, PCA subspaces capture linear variance structure; nonlinear manifold distances such as diffusion maps \citep{conway1996functions} might prove complementary and might better separate disease-state and population-level variance.

Grassmannian geodesic distance provides a geometric test for transportability risk in settings with genuine cross-laboratory or cross-platform distributional shift. Given a new cohort, one can compute its PCA subspace, measure geodesic distances to reference cohorts, and estimate expected classifier degradation from the partial-correlation model---all before training any classifier. The method is most informative when cohorts arise from independent data-generating processes (different laboratories, sequencing protocols, or biological matrices) and least informative when cohorts share a centralized analytical pipeline or when within-cohort variance exceeds between-cohort variance. Code and data are available at \url{https://github.com/elliottower/geometric-transportability}.
