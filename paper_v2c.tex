...
\begin{abstract}
Classifiers trained on one cohort routinely degrade when applied to another, yet quantifying this risk before training requires a measure of cross-cohort distributional distance. We propose geodesic distance between cohort-specific PCA subspaces on the Grassmannian manifold $\mathrm{Gr}(k, d)$ as such a measure and test it on three datasets. Raw geodesic--gap correlations are null or weak ($\rho \leq 0.34$) because the AUC gap conflates source classifier quality with distribution shift. After partialing out source internal AUC, geodesic distance predicts the residual gap on a colorectal cancer microbiome meta-analysis (9 studies, 824 samples; partial $\rho = +0.61$, clustered bootstrap 95\% CI $[-0.02, +0.80]$, 97\% positive; $\Delta R^2 = +0.23$) and on the QMDiab multi-biofluid metabolomics study (3 biofluids $\times$ 3 ethnicities, 356 participants; partial $\rho = +0.40$, clustered CI $[+0.04, +0.73]$). Both results are stable across subspace dimensions $k = 5$--$50$, hold under chordal distance, and replicate with random forest classifiers. A single-study metabolomics dataset (MTBLS7260, 15 plates) remains null, consistent with per-plate AUC noise floors from small case counts ($\sim$12 positives per plate). The confound between source quality and distribution shift likely explains null results in previous geometric prediction attempts.
\end{abstract}
...
We test this idea on three datasets spanning different domains: a colorectal cancer (CRC) microbiome meta-analysis (9 independent studies across 7 countries; \citealp{wirbel2019meta}), a multi-biofluid metabolomics study (QMDiab, 3 biofluids $\times$ 3 ethnicities; \citealp{yousri2015metabolic}), and a single-study metabolomics dataset (MTBLS7260, 15 analytical plates; \citealp{leonletelier2024contributions}).
...
\item \textbf{Noise floor (\S\ref{sec:metabolomics}).} MTBLS7260 remains null: per-plate AUC estimates have SE $\sim 0.10$ from $\sim$12 positives, so neither source quality nor geometry can be resolved.
...
\begin{table}[t]
\centering
\caption{Dataset summary. Cohorts are independent studies (CRC), biofluid--ethnicity groups (QMDiab), or analytical plates (MTBLS7260), each treated as a separate data-generating process.}
\label{tab:cohorts}
...
QMDiab & 9 (3 biofluids $\times$ 3 ethnicities) & 34--200 & 89 & $\sim$50\% & 72 \\
...
The central finding is that geodesic distance on the Grassmannian predicts cross-cohort classifier degradation, provided source classifier quality is first partialed out. The confound between source quality and the AUC gap ($\rho = +0.68$ on the CRC data) masks the geometric signal in raw correlations; once removed, a consistent partial correlation emerges on both multi-cohort datasets and holds under alternative metrics (chordal distance) and classifiers (random forest).
...
The two positive datasets differ in which geometric measure performs best. On the CRC data, geodesic distance is the strongest predictor ($\Delta R^2 = +0.23$), outperforming centroid distance ($\Delta R^2 = +0.08$). On QMDiab, centroid distance dominates (partial $\rho = +0.66$) because the cross-biofluid comparison---plasma versus urine versus saliva---produces large centroid shifts in metabolite concentrations that a simple Euclidean distance captures. This is a harder test than batch correction: different biofluids reflect genuinely different biology, and the geodesic still adds predictive power beyond centroids ($+0.337 \to +0.404$ under partial correlation). The CRC data, where cohorts are drawn from different populations on similar sequencing platforms, provides the cleaner test of subspace-level geometry.

The MTBLS7260 null result provides a useful boundary condition. With $\sim$12 positives per plate, AUC estimates have standard errors of $\sim$0.10, comparable to the total signal range. The confound-correction approach requires that source quality be measurable with reasonable precision; when it is not, partialing out a noisy estimate attenuates rather than sharpens the residual signal.

The directional transport predictor fails in its signed form on all three datasets. Fisher's linear discriminant is estimated from cohort-level data in high-dimensional feature spaces; the resulting direction is too noisy to carry signal. The symmetric geodesic, which aggregates over all $k$ principal angles, averages out per-direction noise and succeeds where single-direction projections fail.

\paragraph{Limitations.} The effective sample size for our correlation analyses is the number of studies ($C = 9$), not the number of pairs ($C(C-1) = 72$), because pairs sharing a source or target are not independent. Our clustered bootstrap accounts for this structure, producing appropriately wide CIs (e.g., $[+0.03, +0.77]$ for CRC with random forest, compared to $[+0.44, +0.75]$ from naive pair-level resampling). Three additional caveats apply. First, all three datasets are retrospective analyses of public data. Second, the QMDiab cohort structure (3 biofluids $\times$ 3 ethnicities) confounds biofluid effects with ethnicity effects for the smallest group (Indian, $n = 34$). Third, PCA subspaces capture linear variance structure; nonlinear manifold distances such as diffusion maps or geodesics on the data manifold itself \citep{conway1996functions} might prove complementary.
